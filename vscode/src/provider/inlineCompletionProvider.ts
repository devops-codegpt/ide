...
const tag: Array<vscode.CancellationToken> = []
...
export default function inlineCompletionProvider(
    ...
    const provider: vscode.InlineCompletionItemProvider = {
        provideInlineCompletionItems: async (
            ...
            console.log("new event!", new Date());
            tag.push(token)
            await sleep()
            tag.pop()
            console.log('len', tag.length, new Date())
            if (tag.length === 0) {
                ...
                //= vscode.workspace
                // .getConfiguration("Codegpt", undefined)
                ...
                if (!editor) {
                    vscode.window.showInformationMessage(
                        "Please open a file first to use CodeGPT."
                    );
                    return;
                }
