...
export function getCodeCompletions(
    ...
    const configuration = workspace.getConfiguration("Codegpt", undefined);
    let server = configuration.get("Server");
    if (mode === "prompt") {
        API_URL = `${server}/multilingual_code_generate_block`;
    } else if (mode === "interactive") {
        API_URL = `${server}/multilingual_code_generate_adapt`;
    } else {
        console.log('--generationPreferenc--', generationPreference)
        if (generationPreference === "line by line") {
            API_URL = `${server}/code/generate`;
        } else {
            API_URL = `${server}/multilingual_code_generate_adapt`;
        }
    }
