...
import changeLLMMode from "./mode/changeLLMMode";
...
export async function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand("codegpt.welcome-page", async () => {
            await welcomePage(context);
        })
    );
    if (vscode.env.isNewAppInstall) {
        vscode.commands.executeCommand("codegpt.welcome-page");
    }
    checkPrivacy();
    survey();
    let targetEditor: vscode.TextEditor;

    const statusBarItemCommandId = "codegpt.disable-enable";
    context.subscriptions.push(
        vscode.commands.registerCommand("codegpt.disable-enable", () => {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand("codegpt.translate-mode", async () => {
            const editor = vscode.window.activeTextEditor;
            if (!editor) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand("codegpt.prompt-mode", () => {
            const editor = vscode.window.activeTextEditor;
            if (!editor) {
                vscode.window.showInformationMessage(localeTag.noEditorInfo);
                return;
    ...
    if (onlyKeyControl) {
        ...
        context.subscriptions.push(
            //vscode.commands.registerCommand('codegpt.inline-completions',()=>
    ...
    vscode.commands.registerCommand("codegpt.new-completions", () => {
        if (oneTimeDispo) {
